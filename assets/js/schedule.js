...
function fetchCall() {
  fetch(url)
    ...
    .finally(() => {
      ...
      console.log();
      contents.innerHTML = `<h2>The CW</h2>`;
...
<p><span>${
              rating[Math.round(element.rating.average)][
                Math.round(element.rating.average)
              ]
            }</span>${element.rating.average}</p>
      ...
      document.querySelectorAll("article").forEach((element) => {
        element.addEventListener("click", function () {
          ...
          <div id="modalRightDiv">
            <h6>${apiData[element.id].name} <span>${
            apiData[element.id].network.name
          }</span></h6>
            ...
            <p>${
              rating[Math.round(apiData[element.id].rating.average)][
                Math.round(apiData[element.id].rating.average)
              ]
            } <span>${apiData[element.id].rating.average}</span></p>
          </div>
`;
...
function Clear(obj) {
  apiData.sort();
  contents.innerHTML = `<h2>${obj.value}</h2>`;
